        ...
        Column(
            ...
            (maxLevel downTo 0).forEach { level ->
                LazyRow(modifier = modifier.align(Alignment.CenterHorizontally)) {
                    items(slotsByLevel[level]!!, key = { slot -> slot.hashCode() }) { slot -> SlotHelper(slot, wand, currentGameState, addAction, isWandDragged) }
                }
            }
        }
    }
...
private fun SlotHelper(slot: Slot, wand: Wand, currentGameState: GameState, addAction: (GameAction) -> Unit, isWandDragged: Boolean) {
    if (null == slot.spell) {
        ...
        Draggable(
            ...
            onDropDidReplaceAction = { replacedSpell -> PlaceSpellAction(wand.id, slot.id, replacedSpell) }) { _, isDragging ->
